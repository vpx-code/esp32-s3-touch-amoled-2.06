...
public:
   ...
   * Not needed for this simple app, but included so the class is a complete
   * template.
   */
...
protected:
  ...
  /*
   * init() runs once when the app is installed (at boot, before the user taps
   * the icon). We use it to configure the Spain timezone so localtime_r()
   * always returns CET/CEST, even without a network connection.
   */
   ...
   * bool cleanResource(void) override;// manual cleanup for non-recorded
   * resources
   */
